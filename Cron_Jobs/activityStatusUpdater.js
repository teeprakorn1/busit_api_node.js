...
const updateActivityStatus = async () => {
  try {
    ...
    const updateToOngoingSql = `
      UPDATE activity 
      SET ActivityStatus_ID = (
        SELECT ActivityStatus_ID 
        FROM activitystatus 
        WHERE ActivityStatus_Name = 'กำลังดำเนินการ'
      )
      WHERE Activity_StartTime <= ? 
      AND Activity_EndTime > ? 
      AND ActivityStatus_ID = (
        SELECT ActivityStatus_ID 
        FROM activitystatus 
        WHERE ActivityStatus_Name = 'เปิดรับสมัคร'
      )
    `;
    ...
    const updateToCompletedSql = `
      ...
      WHERE Activity_EndTime <= ? 
      AND ActivityStatus_ID IN (
        SELECT ActivityStatus_ID 
        FROM activitystatus 
